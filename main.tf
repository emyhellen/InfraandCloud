...
resource "azurerm_resource_group" "teste04infra" {
  name     = "teste04infra"
  location = "eastus"
}
...
resource "azurerm_virtual_network" "vnet" {
  name                = "vnet"
  location            = azurerm_resource_group.teste04infra.location
  resource_group_name = azurerm_resource_group.teste04infra.name
...
resource "azurerm_network_interface" "NetwokInterface" {
  name                = "NetwokInterface"
  location            = azurerm_resource_group.teste04infra.location
  resource_group_name = azurerm_resource_group.teste04infra.name
...
resource "azurerm_network_security_group" "infra-ng" {
  ...
  network_interface_ids = [
    azurerm_network_interface.NetwokInterface.id
  ]
...
resource "azurerm_network_interface_security_group_association" "ng-nic-assoc" {
  network_interface_id      = azurerm_network_interface.NetwokInterface.id
